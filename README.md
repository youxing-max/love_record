...
LOVE_RECORD 是一个为情侣量身定制的恋爱记录系统，帮助您记录和珍藏两个人的美好时光。通过精美的界面设计和丰富的功能模块，让每一个珍贵的瞬间都能被完美保存。

### 快速上手运行（本项目只适合linux系统，目前未实现安装模式，只支持下载运行，故建议可以运行在云服器上！）
...
# ps1:可以更改frontend目录下的pg.jpg文件来修改前端登录界面，否则使用默认！！！
# ps2：云服务器需要放行9998端口，否则无法访问前端界面（云服务厂商控制台修改放行端口并且本地放行:sudo ufw allow 9998/tcp + sudo ufw reload）！！！
```
...
# ps1:可以更改frontend目录下的pg.jpg文件来修改前端登录界面，否则使用默认！！！
# ps2：云服务器需要放行9998端口，否则无法访问前端界面（云服务厂商控制台修改放行端口并且本地放行:sudo ufw allow 9998/tcp + sudo ufw reload）！！！
# ps3：也可以自定义端口：修改\love_record\backend\src\main.cpp文件第47行的端口号即可自定义！
```


### 成品UI展示
![登录界面](./微信图片_2026-01-08_125746_951.png)

![主页1](./微信图片_20260108125818_561_468.png)


![主页2](./微信图片_20260108125818_562_468.png)
