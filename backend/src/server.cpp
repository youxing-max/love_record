...
std::string getProjectRoot() {
    ...
    if (projectRoot.empty()) {
        ...
        if (len != -1) {
            ...
            // 假设可执行文件在 backend/build/ 目录下
            size_t pos = exePath.rfind("/backend/build/");
            if (pos != std::string::npos) {
                projectRoot = exePath.substr(0, pos);
                std::cout << "✅ 项目根目录: " << projectRoot << std::endl;
            } else {
                // 回退方案：尝试向上查找 frontend 目录
                char cwd[1024];
                if (getcwd(cwd, sizeof(cwd)) != nullptr) {
                    std::string currentDir(cwd);
                    std::cout << "📂 当前工作目录: " << currentDir << std::endl;

                    // 检查当前目录是否在 backend/build 下
                    size_t buildPos = currentDir.rfind("/backend/build");
                    if (buildPos != std::string::npos) {
                        projectRoot = currentDir.substr(0, buildPos);
                        std::cout << "✅ 从工作目录推断项目根目录: " << projectRoot << std::endl;
                    } else if (currentDir.rfind("/build") != std::string::npos) {
                        // 可能直接在 build 目录下
                        projectRoot = currentDir.substr(0, currentDir.rfind("/build"));
                        std::cout << "✅ 从 build 目录推断项目根目录: " << projectRoot << std::endl;
                    } else {
                        // 假设当前就在项目根目录
                        projectRoot = currentDir;
                        std::cout << "⚠️  使用当前目录作为项目根目录: " << projectRoot << std::endl;
                    }
                } else {
                    projectRoot = ".";
                    std::cout << "⚠️  无法获取工作目录，使用相对路径" << std::endl;
                }
            }
        } else {
            // 如果无法读取 /proc/self/exe，使用当前工作目录
            char cwd[1024];
            if (getcwd(cwd, sizeof(cwd)) != nullptr) {
                std::string currentDir(cwd);
                std::cout << "📂 当前工作目录: " << currentDir << std::endl;
                ...
                projectRoot = ".";
                std::cout << "⚠️  无法获取路径，使用相对路径" << std::endl;
            }
        }
    }
    return projectRoot;
...
void Server::start() {
    pImpl->running = true;
    std::cout << "🌸 LoveRecord Server Starting..." << std::endl;
    std::cout << "📡 Server running on http://localhost:" << pImpl->port << std::endl;
    std::cout << "💝 Database connected successfully!" << std::endl;
    
    // 从文件加载纪念日数据 - 使用修复后的安全版本
    ...
    std::cout << "🎯 HTTP Server started on port " << pImpl->port << std::endl;
...
std::string Server::handleGetRequest(const std::string& path) {
    std::cout << "📨 收到请求: GET " << path << " HTTP/1.1" << std::endl;
    ...
    if (queryPos != std::string::npos) {
        ...
        std::cout << "🔍 提取路径部分: " << pathOnly << std::endl;
    }
    ...
    if (pathOnly == "/api/photos") {
        ...
        return handleGetStatsRequest();
            ...
            if (pathOnly == "/") {
                ...
                // 如果路径以/frontend/开头，直接映射到frontend目录
                // 例如：/frontend/images/bg.jpg -> /home/youxing/love_record/frontend/images/bg.jpg
                filePath = getProjectRoot() + pathOnly;
            } else {
                // 其他路径映射到 frontend 目录
                filePath = getProjectRoot() + "/frontend" + pathOnly;
            }

            std::cout << "📁 解析文件路径: " << filePath << std::endl;
...
std::string Server::handlePostRequest(const std::string& path, const std::string& body) {
    std::cout << "📨 收到请求: POST " << path << " HTTP/1.1" << std::endl;
    ...
    if (pathOnly == "/api/photos") {
        ...
        response += "\r\n";
        response += "{\"success\": false,\"message\": \"路径不存在\"}";
        return response;
    }
}
...
std::string Server::handlePostPhotosRequest(const std::string& body) {
    ...
    response += "Connection: keep-alive\r\n";
    response += "\r\n";
    response += "{\"success\": true,\"message\": \"照片添加成功\",\"data\": {\"photoId\": 1}}";
    return response;
}
...
std::string Server::handlePhotoUploadRequest(const std::string& body, const std::string& filePath) {
    ...
    if (outFile.is_open()) {
        ...
        // 生成成功响应
            ...
            response += "{\"success\": true,\"message\": \"照片上传成功\",\"data\": {\"photoId\": " + std::to_string(std::time(nullptr)) + ",\"url\": \"" + photoUrl + "\"}}";
        std::cout << "📸 照片上传成功，保存路径: " << filePath << "，URL: " << photoUrl << "，文件大小: " << fileSize << "字节" << std::endl;
    } else {
        response += "{\"success\": false,\"message\": \"无法创建文件\"}";
...
std::string Server::handleDeletePhotosRequest(const std::string& body) {
    ...
    std::cout << "🗑️  收到删除照片请求: ID=" << photoIdStr << ", 名称=" << photoName << std::endl;
        ...
        if (!photoName.empty()) {
            ...
            if (std::filesystem::exists(filePath)) {
                if (std::filesystem::remove(filePath)) {
                    std::cout << "🗑️  照片删除成功，文件路径: " << filePath << std::endl;
                } else {
                    std::cerr << "❌ 无法删除照片文件: " << filePath << std::endl;
                }
            } else {
                std::cerr << "❌ 照片文件不存在: " << filePath << std::endl;
            }
        } else {
            // 根据ID查找文件
            ...
            if (!fileName.empty()) {
                ...
                std::cerr << "❌ 无法找到对应的照片文件，ID=" << photoIdStr << std::endl;
            }
        }
    } catch (const std::filesystem::filesystem_error& e) {
        std::cerr << "❌ 无法操作images目录: " << e.what() << std::endl;
    }
    ...
    response += "Connection: keep-alive\r\n";
    response += "\r\n";
    response += "{\"success\": true,\"message\": \"照片删除成功\"}";
    return response;
}
...
struct Activity {
    ...
    std::string content;
    std::string createdAt;
    std::string operator_; // 操作者字段，使用operator_避免与C++关键字冲突
};
...
void Server::safeLoadActivitiesFromFile() {
    try {
        ...
        if (!std::filesystem::exists(dirPath)) {
            ...
            std::cout << "📁 已创建数据存储目录: " << dirPath << std::endl;
        }
        
        // 检查文件是否存在
        if (!std::filesystem::exists(ACTIVITIES_FILE)) {
            std::cout << "📄 活动数据文件不存在，使用空列表" << std::endl;
            return;
        }
        ...
        if (!inFile.is_open()) {
            std::cerr << "❌ 无法打开活动数据文件: " << ACTIVITIES_FILE << std::endl;
            return;
        }
        ...
        // 限制文件大小，避免内存消耗过高
        if (fileSize > 1024 * 1024) { // 限制为1MB
            std::cerr << "❌ 活动数据文件过大，限制读取大小" << std::endl;
            inFile.close();
            return;
        }
        ...
        // 解析JSON数据 - 简化版，使用更安全的解析方式
        if (jsonContent == "[]" || jsonContent == "") {
            std::cout << "📄 活动数据文件为空，使用空列表" << std::endl;
            return;
        }
        ...
        while (pos < arrayContent.size()) {
            ...
            // 如果找不到匹配的结束括号，跳过
            if (braceCount > 0) {
                std::cerr << "⚠️  找不到匹配的结束括号，跳过该对象" << std::endl;
                pos = objStart + 1;
                continue;
            }
            ...
            if (idPos != std::string::npos) {
                ...
                try {
                    ...
                    std::cerr << "⚠️  ID解析失败，使用默认ID: " << activity.id << std::endl;
                } catch (const std::out_of_range& e) {
                    // ID超出范围，使用当前时间作为默认ID
                    activity.id = static_cast<long long>(std::time(nullptr));
                    std::cerr << "⚠️  ID超出范围，使用默认ID: " << activity.id << std::endl;
                }
            } else {
                // 没有ID字段，使用当前时间作为默认ID
                activity.id = static_cast<long long>(std::time(nullptr));
            ...
            // 1. 所有字段不能为空
            // 2. 不能包含无效字符（如逗号、右括号等）
            // 3. createdAt必须是有效的时间格式（至少包含"-"和"T"）
            if (!activity.content.empty() && !activity.type.empty() && !activity.createdAt.empty() &&
                ...
                std::cout << "📝 加载了一个有效活动: " << activity.content << "，操作者: " << activity.operator_ << std::endl;
            } else {
                std::cerr << "⚠️  跳过无效活动: content='" << activity.content << "', type='" << activity.type << "', createdAt='" << activity.createdAt << "', operator='" << activity.operator_ << "'" << std::endl;
            }
            
            // 查找下一个对象
            pos = objEnd + 2; // 跳过逗号和空格
        }
        
        std::cout << "📥 已从文件加载 " << g_activities.size() << " 个活动数据" << std::endl;
    } catch (const std::exception& e) {
        std::cerr << "❌ 加载活动数据时发生异常: " << e.what() << std::endl;
        // 发生异常时，使用空列表
        g_activities.clear();
    } catch (...) {
        std::cerr << "❌ 加载活动数据时发生未知异常" << std::endl;
        // 发生未知异常时，使用空列表
        g_activities.clear();
    }
}
...
void Server::saveActivitiesToFile() {
    try {
        ...
        if (!outFile.is_open()) {
            std::cerr << "❌ 无法打开活动数据文件进行写入: " << ACTIVITIES_FILE << std::endl;
            return;
        }
        ...
        // 检查写入是否成功
        if (outFile.fail()) {
            std::cerr << "❌ 写入活动数据文件失败" << std::endl;
        } else {
            std::cout << "💾 已将 " << g_activities.size() << " 个活动数据保存到文件: " << ACTIVITIES_FILE << std::endl;
        }
        
        outFile.close();
    } catch (const std::exception& e) {
        std::cerr << "❌ 保存活动数据时发生异常: " << e.what() << std::endl;
    } catch (...) {
        std::cerr << "❌ 保存活动数据时发生未知异常" << std::endl;
    }
}
...
void Server::safeLoadWishesFromFile() {
    try {
        ...
        if (!std::filesystem::exists(WISHES_FILE)) {
            std::cout << "📄 愿望数据文件不存在，使用空列表" << std::endl;
            return;
        }
        ...
        if (!inFile.is_open()) {
            std::cerr << "❌ 无法打开愿望数据文件: " << WISHES_FILE << std::endl;
            return;
        }
        ...
        // 限制文件大小，避免内存消耗过高
        if (fileSize > 1024 * 1024) { // 限制为1MB
            std::cerr << "❌ 愿望数据文件过大，限制读取大小" << std::endl;
            inFile.close();
            return;
        }
        ...
        // 解析JSON数据 - 简化版，使用更安全的解析方式
        if (jsonContent == "[]" || jsonContent == "") {
            std::cout << "📄 愿望数据文件为空，使用空列表" << std::endl;
            return;
        }
        ...
        while (pos < arrayContent.size()) {
                    ...
                    if (objStr[idValueStart] == '"') {
                        ...
                        if (idEnd != std::string::npos) {
                            ...
                            try {
                                ...
                                std::cerr << "⚠️  愿望ID解析失败，使用默认ID: " << wish.id << std::endl;
                            }
                        } else {
                            // 没有找到结尾引号，使用当前时间作为默认ID
                            wish.id = static_cast<long long>(std::time(nullptr));
                            std::cerr << "⚠️  愿望ID解析失败，使用默认ID: " << wish.id << std::endl;
                        }
                    } else {
            ...
            if (!wish.content.empty() && wish.content != "," && wish.content != "\"" && wish.content != "}") {
                ...
                std::cout << "📝 加载了一个有效愿望: " << wish.content << ", 完成状态: " << (wish.isCompleted ? "已完成" : "未完成") << std::endl;
            } else {
                std::cerr << "⚠️  跳过无效愿望: content='" << wish.content << "'" << std::endl;
            }
        ...
        std::cout << "📥 已从文件加载 " << g_wishes.size() << " 个愿望数据" << std::endl;
    } catch (const std::exception& e) {
        std::cerr << "❌ 加载愿望数据时发生异常: " << e.what() << std::endl;
        // 发生异常时，使用空列表
        g_wishes.clear();
    } catch (...) {
        std::cerr << "❌ 加载愿望数据时发生未知异常" << std::endl;
        // 发生未知异常时，使用空列表
        g_wishes.clear();
    }
}
...
void Server::safeLoadDiariesFromFile() {
    try {
        ...
        if (!std::filesystem::exists(DIARIES_FILE)) {
            std::cout << "📄 日记数据文件不存在，使用空列表" << std::endl;
            return;
        }
        ...
        if (!inFile.is_open()) {
            std::cerr << "❌ 无法打开日记数据文件: " << DIARIES_FILE << std::endl;
            return;
        }
        ...
        // 限制文件大小，避免内存消耗过高
        if (fileSize > 1024 * 1024) { // 限制为1MB
            std::cerr << "❌ 日记数据文件过大，限制读取大小" << std::endl;
            inFile.close();
            return;
        }
        ...
        // 解析JSON数据
        if (jsonContent == "[]" || jsonContent == "") {
            std::cout << "📄 日记数据文件为空，使用空列表" << std::endl;
            return;
        }
        ...
        while (pos < arrayContent.size()) {
                ...
                if (colonPos != std::string::npos) {
                    ...
                    if (idValStart != std::string::npos) {
                        ...
                        try {
                            ...
                            std::cerr << "⚠️  日记ID解析失败，使用默认ID: " << diary.id << std::endl;
                        }
                    }
                }
            } else {
            ...
            if (!diary.title.empty() || !diary.content.empty()) {
                ...
                std::cout << "📝 加载了一篇有效日记: " << diary.title << ", 心情: " << diary.mood << std::endl;
            } else {
                std::cerr << "⚠️  跳过无效日记: 标题和内容都为空" << std::endl;
            }
        ...
        std::cout << "📥 已从文件加载 " << g_diaries.size() << " 篇日记数据" << std::endl;
    } catch (const std::exception& e) {
        std::cerr << "❌ 加载日记数据时发生异常: " << e.what() << std::endl;
        // 发生异常时，使用空列表
        g_diaries.clear();
    } catch (...) {
        std::cerr << "❌ 加载日记数据时发生未知异常" << std::endl;
        // 发生未知异常时，使用空列表
        g_diaries.clear();
    }
}
...
void Server::saveDiariesToFile() {
    try {
        ...
        if (!outFile.is_open()) {
            std::cerr << "❌ 无法打开日记数据文件进行写入: " << DIARIES_FILE << std::endl;
            return;
        }
        ...
        // 检查写入是否成功
        if (outFile.fail()) {
            std::cerr << "❌ 写入日记数据文件失败" << std::endl;
        } else {
            std::cout << "💾 已将 " << g_diaries.size() << " 篇日记数据保存到文件: " << DIARIES_FILE << std::endl;
        }
        
        outFile.close();
    } catch (const std::exception& e) {
        std::cerr << "❌ 保存日记数据时发生异常: " << e.what() << std::endl;
    } catch (...) {
        std::cerr << "❌ 保存日记数据时发生未知异常" << std::endl;
    }
}
...
void Server::saveWishesToFile() {
    try {
        ...
        if (!outFile.is_open()) {
            std::cerr << "❌ 无法打开愿望数据文件进行写入: " << WISHES_FILE << std::endl;
            return;
        }
        ...
        // 检查写入是否成功
        if (outFile.fail()) {
            std::cerr << "❌ 写入愿望数据文件失败" << std::endl;
        } else {
            std::cout << "💾 已将 " << g_wishes.size() << " 个愿望数据保存到文件: " << WISHES_FILE << std::endl;
        }
        
        outFile.close();
    } catch (const std::exception& e) {
        std::cerr << "❌ 保存愿望数据时发生异常: " << e.what() << std::endl;
    } catch (...) {
        std::cerr << "❌ 保存愿望数据时发生未知异常" << std::endl;
    }
}
...
void Server::initializeAnniversaryStorage() {
    ...
    if (!std::filesystem::exists(ANNIVERSARIES_FILE)) {
        ...
        if (outFile.is_open()) {
            outFile << "[]";
            outFile.close();
            std::cout << "📄 已创建纪念日数据文件: " << ANNIVERSARIES_FILE << std::endl;
        }
    }
}
...
void Server::safeLoadAnniversariesFromFile() {
    try {
        ...
        // 检查文件是否存在
        if (!std::filesystem::exists(ANNIVERSARIES_FILE)) {
            std::cout << "📄 纪念日数据文件不存在，使用空列表" << std::endl;
            return;
        }
        ...
        if (!inFile.is_open()) {
            std::cerr << "❌ 无法打开纪念日数据文件: " << ANNIVERSARIES_FILE << std::endl;
            return;
        }
        ...
        // 限制文件大小，避免内存消耗过高
        if (fileSize > 1024 * 1024) { // 限制为1MB
            std::cerr << "❌ 纪念日数据文件过大，限制读取大小" << std::endl;
            inFile.close();
            return;
        }
        ...
        // 解析JSON数据 - 简化版，使用更安全的解析方式
        // 只处理简单的JSON结构，避免复杂嵌套导致的问题
        
        // 检查是否为空数组
        if (jsonContent == "[]" || jsonContent == "") {
            std::cout << "📄 纪念日数据文件为空，使用空列表" << std::endl;
            return;
        }
        ...
        size_t end = jsonContent.rfind(']');
        if (start == std::string::npos || end == std::string::npos || start >= end) {
            std::cerr << "❌ 无效的纪念日数据文件格式" << std::endl;
            return;
        }
        ...
        while (pos < arrayContent.size()) {
            ...
            size_t idPos = objStr.find("\"id\":");
            if (idPos != std::string::npos) {
                // 找到id值的开始位置（跳过冒号和可能的空格）
                size_t idValStart = idPos + 6; // "id": 之后的位置
                // 跳过可能的空格
        ...
        std::cout << "📥 已从文件加载 " << g_anniversaries.size() << " 个纪念日数据" << std::endl;
    } catch (const std::exception& e) {
        std::cerr << "❌ 加载纪念日数据时发生异常: " << e.what() << std::endl;
        // 发生异常时，使用空列表
        g_anniversaries.clear();
    } catch (...) {
        std::cerr << "❌ 加载纪念日数据时发生未知异常" << std::endl;
        // 发生未知异常时，使用空列表
        g_anniversaries.clear();
    }
}
...
void Server::saveAnniversariesToFile() {
    ...
    if (!outFile.is_open()) {
        std::cerr << "❌ 无法打开纪念日数据文件进行写入: " << ANNIVERSARIES_FILE << std::endl;
        return;
    }
    ...
    // 检查写入是否成功
    if (outFile.fail()) {
        std::cerr << "❌ 写入纪念日数据文件失败" << std::endl;
    } else {
        std::cout << "💾 已将 " << g_anniversaries.size() << " 个纪念日数据保存到文件: " << ANNIVERSARIES_FILE << std::endl;
    }
...
std::string Server::handleGetAnniversariesRequest() {
    // 构建JSON响应
    std::string jsonStr = "{\"success\": true,\"message\": \"获取纪念日列表成功\",\"data\": {\"anniversaries\": [";
...
std::string Server::handlePostAnniversariesRequest(const std::string& body) {
    // 解析请求体，获取纪念日数据
    std::string title = ""; // 默认标题
    std::string date = ""; // 默认日期
    std::string category = "恋爱"; // 默认分类
    ...
    try {
        ...
        std::cerr << "❌ 解析纪念日请求体时发生异常" << std::endl;
    }
    ...
    // 打印日志
    std::cout << "📅 添加了新纪念日: " << newAnniversary.title << "，日期: " << newAnniversary.date << std::endl;
    ...
    response += "Connection: keep-alive\r\n";
    response += "\r\n";
    response += "{\"success\": true,\"message\": \"纪念日添加成功\",\"data\": {\"anniversaryId\": " + std::to_string(newAnniversary.id) + ",\"title\": \"" + newAnniversary.title + "\",\"date\": \"" + newAnniversary.date + "\",\"category\": \"" + newAnniversary.category + "\",\"createdAt\": \"" + newAnniversary.createdAt + "\"}}";
    return response;
}
...
std::string Server::handleDeleteAnniversariesRequest(const std::string& body) {
    ...
    for (auto it = g_anniversaries.begin(); it != g_anniversaries.end();) {
        if (it->id == anniversaryId) {
            // 打印日志
            std::cout << "🗑️ 删除了纪念日: " << it->title << "，ID: " << it->id << std::endl;
    ...
    if (deleted) {
        response += "{\"success\": true,\"message\": \"纪念日删除成功\"}";
    } else {
        response += "{\"success\": false,\"message\": \"纪念日删除失败，未找到该纪念日\"}";
    }
...
std::string Server::handleGetDiariesRequest() {
    // 构建JSON响应
    std::string jsonStr = "{\"success\": true,\"message\": \"获取日记列表成功\",\"data\": {\"diaries\": [";
...
std::string Server::handlePostDiariesRequest(const std::string& body) {
    // 解析请求体，获取日记数据
    std::string title = ""; // 默认标题
    std::string content = ""; // 默认内容
    int mood = 3; // 默认心情值
    std::string author = "未知作者"; // 默认作者
    
    // 解析请求体，获取日记数据
    try {
        // 打印完整的请求体，用于调试
        std::cout << "📨 收到的日记请求体: " << body << std::endl;
        
        // 解析title字段
        size_t titlePos = body.find("\"title\":");
        std::cout << "🔍 查找title字段，位置: " << titlePos << std::endl;
        
        if (titlePos != std::string::npos) {
            // 找到title字段的冒号，允许字段名和冒号之间有空格
            size_t colonPos = body.find(":", titlePos);
            std::cout << "🔍 查找title冒号，位置: " << colonPos << std::endl;
            ...
            if (colonPos != std::string::npos) {
                ...
                std::cout << "🔍 跳过空格后，title值开始位置: " << afterColon << std::endl;
                
                // 找到title字段值的开始引号
                size_t titleValStart = body.find('"', afterColon);
                std::cout << "🔍 查找title值开始引号，位置: " << titleValStart << std::endl;
                ...
                if (titleValStart != std::string::npos) {
                    ...
                    if (titleValEnd < body.size()) {
                        ...
                        std::cout << "📝 解析到日记标题: " << title << std::endl;
                    } else {
                        std::cerr << "⚠️  未找到title字段的结束引号" << std::endl;
                    }
                }
            }
        }
        ...
        std::cout << "🔍 查找content字段，位置: " << contentPos << std::endl;
        
        if (contentPos != std::string::npos) {
            // 找到content字段的冒号，允许字段名和冒号之间有空格
            size_t colonPos = body.find(":", contentPos);
            std::cout << "🔍 查找content冒号，位置: " << colonPos << std::endl;
            ...
            if (colonPos != std::string::npos) {
                ...
                std::cout << "🔍 跳过空格后，content值开始位置: " << afterColon << std::endl;
                
                // 找到content字段值的开始引号
                size_t contentValStart = body.find('"', afterColon);
                std::cout << "🔍 查找content值开始引号，位置: " << contentValStart << std::endl;
                
                if (contentValStart != std::string::npos) {
                    contentValStart++;
                    
                    // 改进的content字段解析，处理换行符和转义字符
                    size_t contentValEnd = contentValStart;
                    bool inEscape = false;
                    ...
                    if (contentValEnd < body.size()) {
                        ...
                        std::cout << "📝 解析到日记内容: " << (content.size() > 50 ? content.substr(0, 50) + "..." : content) << std::endl;
                    } else {
                        std::cerr << "⚠️  未找到content字段的结束引号" << std::endl;
                    }
                }
            }
        }
        ...
        std::cout << "🔍 查找author字段，位置: " << authorPos << std::endl;
        
        if (authorPos != std::string::npos) {
            // 找到author字段的冒号，允许字段名和冒号之间有空格
            size_t colonPos = body.find(":", authorPos);
            std::cout << "🔍 查找author冒号，位置: " << colonPos << std::endl;
            ...
            if (colonPos != std::string::npos) {
                ...
                std::cout << "🔍 跳过空格后，author值开始位置: " << afterColon << std::endl;
                
                // 找到author字段值的开始引号
                size_t authorValStart = body.find('"', afterColon);
                std::cout << "🔍 查找author值开始引号，位置: " << authorValStart << std::endl;
                
                if (authorValStart != std::string::npos) {
                    authorValStart++;
                    
                    // 改进的author字段解析，处理转义字符
                    size_t authorValEnd = authorValStart;
                    bool inEscape = false;
                    ...
                    if (authorValEnd < body.size()) {
                        ...
                        std::cout << "📝 解析到日记作者: " << author << std::endl;
                    } else {
                        std::cerr << "⚠️  未找到author字段的结束引号，使用默认作者" << std::endl;
                        author = "未知作者"; // 确保作者字段不为空
                    }
                }
            }
        }
    } catch (...) {
        std::cerr << "❌ 解析日记请求体时发生异常" << std::endl;
    }
    ...
    if (!newDiary.title.empty() || !newDiary.content.empty()) {
        ...
        response += "Connection: keep-alive\r\n";
        response += "\r\n";
        response += "{\"success\": false,\"message\": \"日记标题和内容不能为空\"}";
        return response;
    }
    ...
    // 打印日志
    std::cout << "📝 添加了新日记: " << newDiary.title << ", 作者: " << newDiary.author << std::endl;
    ...
    response += "Connection: keep-alive\r\n";
    response += "\r\n";
    response += "{\"success\": true,\"message\": \"日记添加成功\",\"data\": {\"diaryId\": " + std::to_string(newDiary.id) + ",\"title\": \"" + newDiary.title + "\",\"content\": \"" + newDiary.content + "\",\"mood\": " + std::to_string(newDiary.mood) + ",\"author\": \"" + newDiary.author + "\",\"createdAt\": \"" + newDiary.createdAt + "\"}}";
    return response;
}
...
std::string Server::handleDeleteDiariesRequest(const std::string& body) {
    ...
    if (!idStr.empty()) {
        try {
            ...
            std::cerr << "❌ 日记ID解析失败: " << idStr << std::endl;
        }
    }
    
    // 查找并删除日记
    bool deleted = false;
    std::string diaryTitle = "未知日记";
    for (auto it = g_diaries.begin(); it != g_diaries.end();) {
        if (it->id == diaryId) {
            diaryTitle = it->title;
            
            // 打印日志
            std::cout << "🗑️ 删除了日记: " << it->title << "，ID: " << it->id << std::endl;
    ...
    if (deleted) {
        ...
        response += "{\"success\": false,\"message\": \"日记删除失败，未找到该日记\"}";
    }
...
std::string Server::handleGetWishesRequest() {
    // 构建JSON响应
    std::string jsonStr = "{\"success\": true,\"message\": \"获取愿望列表成功\",\"data\": {\"wishes\": [";
    
    // 添加所有愿望
    for (size_t i = 0; i < g_wishes.size(); i++) {
        const Wish& wish = g_wishes[i];
        // 添加title字段，与content字段值相同，适配前端需求
...
std::string Server::handlePostWishesRequest(const std::string& body) {
    ...
    try {
        ...
        std::cerr << "❌ 解析愿望请求体时发生异常" << std::endl;
    }
    ...
    // 打印日志
    std::cout << "✨ 添加了新愿望: " << newWish.content << std::endl;
    ...
    response += "Connection: keep-alive\r\n";
    response += "\r\n";
    response += "{\"success\": true,\"message\": \"愿望添加成功\",\"data\": {\"wishId\": " + std::to_string(newWish.id) + ",\"content\": \"" + newWish.content + "\",\"isCompleted\": 0,\"createdAt\": \"" + newWish.createdAt + "\"}}";
    return response;
}
...
std::string Server::handleCompleteWishRequest(const std::string& body) {
    ...
    // 解析isCompleted参数
    bool isCompleted = true; // 默认标记为完成
    size_t completedPos = body.find("\"isCompleted\":");
    if (completedPos != std::string::npos) {
        // 找到isCompleted字段的值开始位置，跳过":"和可能的空格
    ...
    for (auto& wish : g_wishes) {
        if (wish.id == wishId) {
            ...
            if (isCompleted) {
                ...
                // 打印日志
                std::cout << "🎉 完成了愿望: " << wish.content << "，ID: " << wish.id << std::endl;
            } else {
                // 清空完成时间
                wish.completedAt = "";
                
                // 打印日志
                std::cout << "🔄 取消完成愿望: " << wish.content << "，ID: " << wish.id << std::endl;
            }
    ...
    if (found) {
        // 使用std::string来构建响应，避免直接拼接const char*和const char[]
        std::string message = isCompleted ? "愿望标记为完成成功" : "愿望取消完成成功";
        response += "{\"success\": true,\"message\": \"" + message + "\"}";
    } else {
        response += "{\"success\": false,\"message\": \"未找到该愿望\"}";
    }
...
std::string Server::handleDeleteWishesRequest(const std::string& body) {
    ...
    for (auto it = g_wishes.begin(); it != g_wishes.end();) {
        if (it->id == wishId) {
            ...
            // 打印日志
            std::cout << "🗑️ 删除了愿望: " << wishContent << "，ID: " << it->id << std::endl;
    ...
    if (deleted) {
        response += "{\"success\": true,\"message\": \"愿望删除成功\"}";
    } else {
        response += "{\"success\": false,\"message\": \"愿望删除失败，未找到该愿望\"}";
    }
    
    return response;
}

// 以下是server.h中声明但实际未使用的函数，添加空实现以避免链接错误
...
std::string Server::handleLoveDaysRequest() {
    ...
    response += "Connection: keep-alive\r\n";
    response += "\r\n";
    response += "{\"success\": true,\"message\": \"获取恋爱天数成功\",\"data\": {\"days\": 100}}";
    return response;
}
...
std::string Server::handleGetActivitiesRequest() {
    ...
    // 构建JSON响应
    std::string jsonStr = "{\"success\": true,\"message\": \"获取活动列表成功\",\"data\": {\"activities\": [";
...
std::string Server::handlePostActivitiesRequest(const std::string& body) {
    // 解析请求体，获取活动数据
    std::string type = "other"; // 默认类型
    std::string content = "";
    std::string createdAt = "";
    std::string operator_ = "";
    long long id = std::time(nullptr);
    
    std::cout << "📨 收到活动请求体: " << body << std::endl;
    ...
    try {
        ...
        if (idStart != std::string::npos) {
            idStart = body.find_first_of("0123456789", idStart + 6); // 找到id值的开始
            if (idStart != std::string::npos) {
                size_t idEnd = body.find_first_not_of("0123456789", idStart); // 找到id值的结束
                if (idEnd != std::string::npos) {
        ...
        std::cerr << "❌ 解析活动请求体时发生异常" << std::endl;
    }
    ...
    // 确保内容有效
    if (content.empty() || content == ",") {
        // 只有在content确实为空或无效时才进行替换
        content = "未知活动";
    }
    
    // 创建新的活动对象
    Activity newActivity;
    newActivity.id = id; // 使用前端传递的ID或生成的唯一ID
    newActivity.type = type;
    newActivity.content = content;
    newActivity.createdAt = createdAt;
    // 如果操作者为空，默认使用"未知用户"
    newActivity.operator_ = operator_.empty() ? "未知用户" : operator_;
    ...
    if (!newActivity.content.empty() && !newActivity.type.empty() && !newActivity.createdAt.empty() &&
        ...
        // 添加到全局活动列表的开头，确保最新的活动在前面
        g_activities.insert(g_activities.begin(), newActivity);
        
        // 限制活动列表数量，只保留最近的50个活动
        if (g_activities.size() > 50) {
            g_activities.pop_back();
        }
        
        std::cout << "✅ 已添加有效活动: " << newActivity.content << "，类型: " << newActivity.type << "，操作者: " << newActivity.operator_ << std::endl;
    } else {
        std::cerr << "❌ 拒绝添加无效活动: content='" << newActivity.content << "', type='" << newActivity.type << "', createdAt='" << newActivity.createdAt << "', operator='" << newActivity.operator_ << "'" << std::endl;
    }
    ...
    // 打印日志
    std::cout << "📝 添加了新活动: " << newActivity.content << "，类型: " << newActivity.type << "，操作者: " << newActivity.operator_ << "，ID: " << newActivity.id << std::endl;
    ...
    response += "Connection: keep-alive\r\n";
    response += "\r\n";
    response += "{\"success\": true,\"message\": \"活动添加成功\",\"data\": {\"activityId\": " + std::to_string(newActivity.id) + ",\"type\": \"" + newActivity.type + "\",\"content\": \"" + newActivity.content + "\",\"createdAt\": \"" + newActivity.createdAt + "\",\"operator\": \"" + newActivity.operator_ + "\"}}";
    return response;
}
...
std::string Server::handleGetStatsRequest() {
    ...
    std::cout << "📊 当前g_wishes大小: " << g_wishes.size() << std::endl;
    ...
    response += "{\"success\": true,\"message\": \"获取统计数据成功\",\"data\": {";
...
std::string Server::serveStaticFile(const std::string& filePath) {
    std::cout << "📄 尝试读取文件: " << filePath << std::endl;
    std::cout << "📂 尝试打开静态文件: " << filePath << std::endl;
    ...
    if (std::filesystem::is_directory(path)) {
        std::cerr << "❌ 无法打开目录作为文件: " << filePath << std::endl;
        std::string response = "HTTP/1.1 404 Not Found\r\n";
        response += "Content-Type: text/html; charset=UTF-8\r\n";
        response += "Access-Control-Allow-Origin: *\r\n";
        response += "Connection: close\r\n";
        response += "\r\n";
        response += "<!DOCTYPE html><html><body><h1>404 Not Found</h1><p>目录无法直接访问</p></body></html>";
        return response;
    }
    ...
    if (!std::filesystem::exists(path) || !std::filesystem::is_regular_file(path)) {
        ...
        response += "Connection: close\r\n";
        response += "\r\n";
        response += "<!DOCTYPE html><html><body><h1>404 Not Found</h1><p>文件不存在</p></body></html>";
        return response;
    }
    ...
    if (!file.is_open()) {
        ...
        response += "Connection: close\r\n";
        response += "\r\n";
        response += "<!DOCTYPE html><html><body><h1>404 Not Found</h1><p>文件无法访问</p></body></html>";
        return response;
    }
    ...
    try {
        ...
        if (!file.read(buffer.data(), size)) {
            ...
            response += "Connection: close\r\n";
            response += "\r\n";
            response += "<!DOCTYPE html><html><body><h1>500 Internal Server Error</h1><p>读取文件失败</p></body></html>";
            return response;
        }
        ...
        std::cout << "✅ 成功返回静态文件: " << filePath << "，大小: " << size << "字节" << std::endl;
        return response;
        ...
        response += "Connection: close\r\n";
        response += "\r\n";
        response += "<!DOCTYPE html><html><body><h1>500 Internal Server Error</h1><p>内存分配失败</p></body></html>";
        return response;
    } catch (const std::exception& e) {
        ...
        response += "Connection: close\r\n";
        response += "\r\n";
        response += "<!DOCTYPE html><html><body><h1>500 Internal Server Error</h1><p>处理文件时发生异常</p></body></html>";
        return response;
    }
}
...
std::map<std::string, std::string> Server::parseHttpRequest(const std::string& request) {
    ...
    while (std::getline(requestStream, line) && line != "\r") {
        ...
        if (colonPos != std::string::npos) {
            ...
            std::string value = line.substr(colonPos + 2, line.size() - colonPos - 3); // 去掉冒号、空格和\r
    ...
    if (!body.empty()) {
        parsedRequest["body"] = body.substr(0, body.size() - 1); // 去掉最后一个\n
    ...
    while (pImpl->running) {
        ...
        if (isFileUpload) {
            ...
            if (outFile.is_open()) {
                ...
                // 读取所有剩余的请求体并写入文件，确保完整保存图片内容
                char bodyBuffer[8192] = {0};
                ssize_t bodyBytesRead;
                
                // 使用阻塞模式读取数据，确保完整读取文件内容
                // 设置超时时间为30秒
                ...
                // 循环读取所有数据，直到没有数据可读或超时
                while ((bodyBytesRead = read(new_socket, bodyBuffer, sizeof(bodyBuffer))) > 0) {
                    // 写入文件
                    outFile.write(bodyBuffer, bodyBytesRead);
                }
                
                // 检查是否是正常读取结束（bodyBytesRead == 0）或超时/错误
                if (bodyBytesRead < 0) {
                    // 检查是否是超时错误
                    if (errno == EWOULDBLOCK || errno == EAGAIN) {
                        std::cerr << "⚠️  读取文件内容超时，但已保存部分内容" << std::endl;
                    } else {
                        std::cerr << "❌ 读取文件内容失败: " << strerror(errno) << std::endl;
                    }
                }
            ...
            if (requestMethod == "POST") {
                ...
                if (contentLengthPos != std::string::npos) {
                    ...
                    try {
                        ...
                        std::cerr << "❌ 解析Content-Length失败" << std::endl;
                    }
                }
            }
        ...
        if (bytesSent < 0) {
            ...
            std::cout << "✅ 发送响应成功，字节数: " << bytesSent << std::endl;
        }
